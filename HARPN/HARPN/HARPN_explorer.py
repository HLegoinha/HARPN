...
def HARPN(path_donwload_folder, PATH_DRIVER, target, fiber=None, file_type=None, folder=None, download=True):
    ...
    if PATH_DRIVER[(len(PATH_DRIVER)-12):] == 'chromedriver':
        ...
        browser = webdriver.Chrome(options=chrome_options, executable_path= PATH_DRIVER) #<- !! INSERT PATH TO CHROME DRIVER
    ...
    if PATH_DRIVER[(len(PATH_DRIVER)-11):] == 'geckodriver':
        ...
        browser = webdriver.Firefox(executable_path = PATH_DRIVER )   #<- !! INSERT PATH TO GECKO DRIVER
        browser.get('http://archives.ia2.inaf.it/tng/faces/search.xhtml?dswid=3397')
